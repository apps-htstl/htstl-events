// app/(poojari)/[listId]/index.tsx
// Event Picker — fetches the Google Sheet CSV, extracts distinct events from
// the sheet's event column, and shows seen/unseen counts per event.
...
import { SevaList, SevaEntry, SevaProgress } from '@/lib/types';
...
// Build SevaEntry objects from raw CSV
...
export default function EventPickerScreen() {
  ...
  useEffect(() => {
    ...
    getSevaProgress(appUser.orgId, listId, appUser.uid).then((progress) => {
      if (progress) {
        setSeenRowKeys(new Set(progress.seenRowKeys));
      }
    });
  ...
  const fetchSheet = useCallback(async (list: SevaList) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to load sheet. Make sure it is shared publicly.');
      setIsLoading(false);
    } finally {
  ...
  // Fetch sheet when seva list is resolved
  useEffect(() => {
    if (sevaList) {
      fetchSheet(sevaList);
    }
  ...
  // Recompute events summary whenever entries or seen keys change
  useEffect(() => {
    const eventMap = new Map<string, EventSummary>();
    for (const entry of entries) {
      const name = entry.eventValue || '(No Event)';
      if (!eventMap.has(name)) {
        eventMap.set(name, { name, totalCount: 0, seenCount: 0 });
      }
      const ev = eventMap.get(name)!;
      ev.totalCount++;
      if (seenRowKeys.has(entry.rowKey)) ev.seenCount++;
    }
    setEvents(Array.from(eventMap.values()).sort((a, b) => a.name.localeCompare(b.name)));
  }, [entries, seenRowKeys]);
  ...
  const renderEvent = ({ item }: { item: EventSummary }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          router.push({
            pathname: `/(poojari)/${listId}/cards` as any,
        ...
        <View style={styles.eventBody}>
          ...
          <View style={styles.progressRow}>
            <View style={styles.progressBar}>
              <View
                style={[
                  styles.progressFill,
                  {
                    width: `${item.totalCount > 0 ? (item.seenCount / item.totalCount) * 100 : 0}%`,
                  } as any,
            ...
            <Text style={styles.progressText}>
              {item.seenCount}/{item.totalCount} seen
            </Text>
          </View>
          {!allDone && (
            <Text style={styles.unseenText}>
              {unseen} {unseen === 1 ? 'person' : 'people'} remaining
            </Text>
          )}
        </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {isLoading ? (
        <View style={styles.center}>
          ...
          <Text style={styles.loadingText}>Loading seva entries from Google Sheet…</Text>
        </View>
      ) : error ? (
        <View style={styles.center}>
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.errorText}>
            The sheet is empty or the event column "{sevaList?.eventColumn}" was not found.
          </Text>
        </View>
      ) : (
        <>
          <View style={styles.summaryBanner}>
            ...
            <Text style={styles.summaryText}>
              {entries.length} total entries across {events.length} event{events.length !== 1 ? 's' : ''}
            </Text>
          </View>
          <FlatList
            data={events}
